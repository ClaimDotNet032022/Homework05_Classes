...
namespace ClassesHomework
{
    ...
    public class ClassesHomework
    {
        ...
        {
            // You should be doing these from the top down anyway,
            // but this test MUST be done last.

            // Goal 1: Uncomment the code in the Zoo.cs file to create the Zoo class.
            // Don't change anything in the Zoo.cs besides uncommenting. If all
            // the above goals have been fulfilled, then the below tests will pass.
            // Contraints: You are not allowed to change this code or the code in the Zoo class.
            // If anything needs changed, edit types already created.
            Zoo zoo = new Zoo("Zoo Name");
            Cat sylvester = new Cat();
            sylvester.CelebrateBirthday();
            zoo.AddAnimal("Sylvester", sylvester);
            Cat heathcliff = new Cat();
            heathcliff.CelebrateBirthday();
            heathcliff.CelebrateBirthday();
            zoo.AddAnimal("Heathcliff", heathcliff);
            Dog pluto = new Dog();
            pluto.CelebrateBirthday();
            pluto.CelebrateBirthday();
            pluto.CelebrateBirthday();
            zoo.AddAnimal("Pluto", pluto);
            Dog rinTinTin = new Dog("German shepherd", 3);
            zoo.AddAnimal("Rin Tin Tin", rinTinTin);

            zoo.SpeakNOW();
            // This kind of assert I'm using below works better
            // for comparing double/float. As long as the values
